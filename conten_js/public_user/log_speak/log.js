...
const mockServer = {
    // 从本地存储加载数据
    initData() {
        const savedArticles = localStorage.getItem('mock_Page_sockes');
        if (savedArticles) {
            try {
                this.data.articles = JSON.parse(savedArticles);
                console.log('从本地存储加载文章数据成功');
            } catch (error) {
    ...
    saveData() {
        try {
            localStorage.setItem('mock_Page_sockes', JSON.stringify(this.data.articles));
    ...
    updateArticle(id, updatedArticle) {
        return new Promise(resolve => {
            setTimeout(() => {
                ...
                this.saveData(); // 保存到本地存储
                resolve({ status: 200, data: this.data.articles[index] });
            }, 700); // 模拟网络延迟
        });
    },
    ...
    deleteArticle(id) {
        return new Promise(resolve => {
            setTimeout(() => {
                ...
                this.saveData(); // 保存到本地存储
                resolve({ status: 200, data: deletedArticle });
            }, 600); // 模拟网络延迟
        });
    }
};
